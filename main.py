...
class App(QMainWindow):
    ...
    def create_home_tab(self):
        ...
        control_layout.addWidget(self.host_dropdown)
        ...
        layout.addWidget(control_frame, 1) # Control frame takes 1 part of space
        layout.addWidget(log_frame, 3)    # Log frame takes 3 parts of space
    ...
    def get_selected_host(self):
        selected_host_name = self.host_dropdown.currentText()
    ...
    def check_for_updates_on_startup(self):
        ...
        self.update_worker.finished.connect(lambda success, msg: self._handle_update_check_result(success, msg, startup_check=True))
        self.update_worker.start()

    def check_for_updates_gui(self):
        self.log("Manually checking for updates...")
        self.update_worker = Worker(um.check_for_updates)
        self.update_worker.finished.connect(lambda success, msg: self._handle_update_check_result(success, msg, startup_check=False))
        self.update_worker.start()

    def _handle_update_check_result(self, update_available, latest_version_assets, startup_check):
        if update_available:
            latest_version, assets = latest_version_assets # Unpack the tuple
